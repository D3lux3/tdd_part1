...
function createApp(database: Database) {
  ...
  app.get("/prices", (req, res) => {
    ...
    const date2 = parseDate2(req.query.date as string);
  ...
  function calculateReduction(date: Date | undefined, date2: Temporal.PlainDate | undefined) {
    let reduction = 0;
    if (date && isMonday(date, date2) && !isHoliday(date2)) {
      reduction = 35;
    }
    return reduction;
  }

  function isMonday(date: Date, date2: Temporal.PlainDate | undefined) {
    return date.getDay() === 1 && date2 && date2.dayOfWeek == 1;
  }
  ...
  function isHoliday(date: Temporal.PlainDate | undefined) {
    ...
    for (let row of holidays) {
      ...
      if (
        date && holiday && date.equals(holiday)
      ) {
        return true;
      }
    }
